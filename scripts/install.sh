...
init (){
        ...
        bash -c "$(curl -SsL https://docs.hxq1990.top/scripts/k8s-init.sh)"
}
...
conf_docker (){
        ...
        bash -c "$(curl -SsL https://docs.hxq1990.top/scripts/docker.sh)" @ $1
}

master (){
        echo "Initializing master node with api_ip=$1, pod_ip=$2, svc_ip=$3..."
        bash -c "$(curl -SsL https://docs.hxq1990.top/scripts/kubeadm-init.sh)" @ $1 $2 $3
...
flannel (){
        ...
        source .bash_profile
        bash -c "$(curl -SsL https://docs.hxq1990.top/scripts/kube-flannel.sh)" @ $1
...
istio (){
        ...
        source ~/.bash_profile
        [ -e $PWD/istio-1.9.9-edit.tar.gz ] || wget https://docs.hxq1990.top/scripts/istio-1.9.9-edit.tar.gz
...
conf_istio (){
        ...
        source .bash_profile
        bash -c "$(curl -SsL https://docs.hxq1990.top/scripts/gateway.sh)"
        bash -c "$(curl -SsL https://docs.hxq1990.top/scripts/virtualservice.sh)"
        bash -c "$(curl -SsL https://docs.hxq1990.top/scripts/gateway-gzip.sh)"
...
kuboard (){
        ...
        source .bash_profile
        bash -c "$(curl -SsL https://docs.hxq1990.top/scripts/kuboard.sh)"
        bash -c "$(curl -SsL https://docs.hxq1990.top/scripts/metrics-server.sh)"
...
limit (){
        ...
        source .bash_profile
        # 限制容器资源
        bash -c "$(curl -SsL https://docs.hxq1990.top/scripts/limit.sh)" @ $1
        kubectl create ns $1
...
zabbix (){
        ...
        [ -e $PWD/zabbix-agent-5.0.17-1.el7.x86_64.rpm ] || wget https://docs.hxq1990.top/scripts/zabbix-agent-5.0.17-1.el7.x86_64.rpm
        bash -c "$(curl -SsL https://docs.hxq1990.top/scripts/zabbix.sh)" @ $1
}

alertmanager (){
        echo "Installing AlertManager and kube-state-metrics..."
        source .bash_profile
        bash -c "$(curl -SsL https://docs.hxq1990.top/scripts/alertmanager.sh)"
...
cert (){
        ...
        wget -O /scripts/cert.sh https://docs.hxq1990.top/scripts/cert.sh
...
del_images (){
        ...
        [ -d /scripts ] || mkdir /scripts
        wget -O /scripts/clear_images.sh https://docs.hxq1990.top/scripts/clear_images.sh
...
kube-state-metrics (){
        ...
        source .bash_profile
        bash -c "$(curl -SsL https://docs.hxq1990.top/scripts/kube-state-metrics.sh)"
        kubectl apply -f kube-state-metrics.yaml
}

install_nginx (){
        curl -Ss -O https://docs.hxq1990.top/scripts/nginx.yaml
